...
class RoundtripToTargetNode(Node):
    def __init__(self):
        ...
        #Actuator parameters
        self.declare_parameter("actuator_topic", "/actuator_cmd")
        self.declare_parameter("actuator_extend_sec", 10.0)
        self.declare_parameter("actuator_retract_sec", 10.0)
        self.declare_parameter("post_actuator_settle_sec", 2.0)
        ...
        self.actuator_extend_sec = float(self.get_parameter("actuator_extend_sec").value)
        self.actuator_retract_sec = float(self.get_parameter("actuator_retract_sec").value)
        self.post_actuator_settle_sec = float(self.get_parameter("post_actuator_settle_sec").value)

        self.status_pub = self.create_publisher(String, self.get_parameter("status_topic").value, 10)
        self.actuator_pub = self.create_publisher(Float32, self.get_parameter("actuator_topic").value, 10)
    ...
    def command_actuator(self, value):
        msg = Float32()
        msg.data = float(value)
        self.actuator_pub.publish(msg)

        if value > 0:
            self.publish_status("actuator_extending")
        elif value < 0:
            self.publish_status("actuator_retracting")
        else:
            self.publish_status("actuator_stopped")
    ...
    def start_extend_actuator(self):
        self.cancel_timer()
        self.phase = "extend_actuator"
        self.publish_status("extending_actuator")
        self.command_actuator(1.0)
        self.timer = self.create_timer(self.actuator_extend_sec, self.stop_after_extend)

    def stop_after_extend(self):
        self.cancel_timer()
        self.command_actuator(0.0)

        self.phase = "wait_at_target"
        self.publish_status(f"waiting_{self.wait_at_target_sec:.1f}_sec_at{self.target_name}")
        self.timer = self.create_timer(self.wait_at_target_sec, self.start_retract_actuator)

    def start_retract_actuator(self):
        self.cancel_timer()
        self.phase = "retract_actuator"
        self.publish_status("retracting_actuator")
        self.command_actuator(-1.0)
        self.timer = self.create_timer(self.actuator_retract_sec, self.stop_after_retract)

    def stop_after_retract(self):
        self.cancel_timer()
        self.command_actuator(0.0)

        self.phase = "post_actuator_turn"
